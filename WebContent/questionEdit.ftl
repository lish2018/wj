...
<html lang="zh-CN">
		...
		<div style="width:1140px; background-color:white; margin:20px auto 100px; padding:10px;box-shadow: #ccc 0 0 20px;">
			...
			<div class="questionTab_div">
				<div style="float: left;font-size: 16px;">
					<span class="">问卷标题：</span><input id="wj_title" value="${questionnaire.title}"/>
				...
				<div style="float: right;">
					...
					<#else>
					<a class="btn lightblue noAdd">保存</a>&nbsp;
					</#if>
	...
	<script>
		...
		<#if questionnaire.status!=0>
			layer.alert("问卷处于发布或已截止状态，只允许查看，不允许编辑");
		</#if>
		...
		wj.id=${questionnaire.id};
		wj.title="${questionnaire.title}";
		wj.type=${questionnaire.type};
		...
		$.post("../editQuestion", {id:${questionnaire.id}}, function(inObj){
		...
		$("#questionnaire_save").on("click", function() {
			...
			layer.confirm('是否保存数据？', {
			  btn: ['保存','取消'] 
				  ...
				  $.post("../editQuestions",{json:JSON.stringify(wj)},function(res){
					res = JSON.parse(res);
					layer.msg(res.msg);
		...
		$(".testlist").on("click",".delete", function(){
			...
			$.post("../deleteOneQuestion",{id:id},function(res){
				res = JSON.parse(res);
				layer.msg(res.msg);
			})
			console.log(id);
