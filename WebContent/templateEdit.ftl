		...
		<div style="width:1140px; background-color:white; margin:20px auto 100px; padding:10px;box-shadow: #ccc 0 0 20px;">
			...
			<div class="questionTab_div">
				<div style="float: left;font-size: 16px;">
					<a class="question_title">编辑模板</a> &nbsp;<span class="arrow"></span>&nbsp;
					<span style="cursor:pointer;">${templateData.title}</span>
				</div>
	...
	<script>
		...
		$("#questionnaire_save").on("click", function() {
			...
			layer.confirm('确认保存吗？', {
				...
				$.post("${request.contextPath}/admin/template/addTemplateQuestions",{json:JSON.stringify(wj)},function(res){
					onbeforeunload = null;
					res = JSON.parse(res);
					layer.msg(res.msg);
					setTimeout(function(){
